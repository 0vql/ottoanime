  ...
  return Data?.length > 0 ? (
    <>
      <div className="w-10/12 my-5 relative">
        <span
          className={heading === "Ep" ? "text-white" :`${theme.text.selected} px-2 flex  font-bold items-center  text-2xl md:text-3xl`}
        >
          {/* {Icon ? (
            <Icon
              size={15}
      ...
      <div className="grid grid-cols-3  w-full px-2 my-6  gap-2  justify-center  md:grid-cols-4 xl:grid-cols-6 lg:px-16 lg:my-16 lg:mt-0  2xl:grid-cols-6 xl:gap-[1rem]">
        {Data?.map((item, index) => ( 
