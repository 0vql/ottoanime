...
<motion.div initial={{x:"100vh"}} animate={{x:0,transition:{duration:0.2}}} exit={{x:"100vh"}} className={`fixed  h-screen p-4 overflow-y-auto  z-50 w-80 ${theme.background}  transition-all right-0 top-0 `} >
   ...
   <form action="#" className="mb-6">
      <div className="mb-3">
         ...
         <input type="email" id="email" className="bg-neutral-900 border border-gray-800 outline-none p-3 w-full text-gray-200 text-sm rounded-sm " placeholder="name@gmail.com" required="" />
      </div>
      <div className="mb-3">
         <label htmlFor="subject" className={`block mb-2 text-sm font-bold ${theme.text.selected}`}>Subject</label>
         <input type="text" id="subject" className="bg-neutral-900 border border-gray-800 outline-none p-3 w-full text-gray-200 text-sm rounded-sm " placeholder="Let us know how we can help you" required="" />
      </div>
      <div className="mb-3">
         <label htmlFor="message" className={`block mb-2 text-sm font-bold ${theme.text.selected}`}>Your message</label>
         <textarea id="message" rows="4" className="block p-2.5 w-full bg-neutral-900 text-sm text-white bg-gray-150 rounded-lg border border-gray-800 " placeholder="Your message..."></textarea>
      </div>
